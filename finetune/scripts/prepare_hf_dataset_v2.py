...
def load_dataset_from_json(
    ...
    print(f"Expanding records: multiple questions per page...")
    ...
    for record_idx, record in enumerate(data, 1):
        ...
        if not all([filename, page_number is not None, questions, total_pages is not None]):
            skipped_count += 1
            continue
        ...
        # Create samples: one per question
        for q_idx, q_obj in enumerate(questions):
            query_text = q_obj.get("question")
            hint = q_obj.get("hint", "")
            ...
            samples.append({
                ...
                "doc_id": doc_id,
                "query": query_text,
                "query_type": f"q{q_idx+1}",
                "query_explanation": hint,
...
if __name__ == "__main__":
    load_dataset_from_json(
        json_path="finetune/scripts/visual_qa_responses_v2.json",
        pdf_base_path="finetune/training_documents",
        train_test_split=0.98,
        output_dir="finetune/datasets/visual_queries_dataset_v2",
        dpi=100,
        image_format="jpg",
        jpeg_quality=90,
    )
