"""
Chunk annotated pages into text and figure chunks for VQA dataset.
Saves the chunked data into a CSV file for further processing.
Note: Without filter_qa_pairs because the filtering is already done in the input JSON.
"""
...
# CONFIG
# ---------------------------------------------------------------------------
JSON_PATH = Path("data/annotations/final_annotations/filtered_annotations/by_category/second_pass_classified_qa_category_A.json")
IMG_DIR = Path("data/pages")
CSV_PATH = Path("src/dataset/chunks/second_pass/chunked_pages_second_pass.csv")
STRATEGY = "hi_res"
LIMIT = -1
...
def main() -> None:
    ...
    for rec in tqdm(records, desc="Processing records"):
        ...
        # Process QA pairs from qa_pairs array
        qa_list = rec.get("qa_pairs", [])
        for qa in qa_list:
            query = extract_value(qa.get("question"))
            answer = extract_value(qa.get("answer"))
            hint = qa.get("hint", "")
