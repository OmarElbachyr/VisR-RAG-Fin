...
class ColQwen25Retriever(BaseRetriever):
    ...
    def _embed_images(self, images: list[Image.Image], batch_size: int) -> torch.Tensor:
        ...
        for i in range(0, len(images), batch_size):
            ...
            with torch.no_grad():
                emb = self.model(**inputs)  # shape [B, D]
            embs.append(emb)
    ...
    def search(
        ...
        for i in range(0, len(q_texts), batch_size):
            ...
            inputs = {k: v.to(self.device) for k, v in inputs.items()}
            with torch.no_grad():
                q_emb = self.model(**inputs)  # shape [B, D]

            scores_tensor = self.processor.score_multi_vector(q_emb, self.page_embeddings)  # [B, P]
